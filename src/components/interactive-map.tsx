...
import { getCategoryKeyByName, getCategoryColorByKey, getCategoryIcon } from '@/lib/category-colors';
...
export default function InteractiveMap({ locations, filteredNews, onLocationSelect, useCustomIcons }: InteractiveMapProps) {
  ...
  const createCategoryIcon = (category: string, color: string) => {
    ...
    // Debug logging
    console.log('Creating icon for category:', category, 'iconName:', iconName, 'color:', color);
    ...
    try {
      ...
      console.error('Error creating category icon:', error, { category, iconName, color });
  ...
  const getIconPath = (iconName: string): string => {
    ...
    const iconPaths: Record<string, string> = {
      ...
      'pistol': 'M30 40C27.25 40 25 42.25 25 45V65C25 67.75 27.25 70 30 70H35V75C35 77.75 37.25 80 40 80H50C52.75 80 55 77.75 55 75V70H80C85.55 70 90 65.55 90 60V50C90 44.45 85.55 40 80 40H70V35C70 32.25 67.75 30 65 30H60V25C60 22.25 57.75 20 55 20H45C42.25 20 40 22.25 40 25V30H35C32.25 30 30 32.25 30 35V40ZM35 40H60V35H55V30H45V35H40V40H35ZM35 50H80V60H35V50Z',
      'shield-star': 'M60 15L30 25V50C30 70 45 87.5 60 95C75 87.5 90 70 90 50V25L60 15ZM60 30L65 40H75L67.5 47.5L70 57.5L60 52.5L50 57.5L52.5 47.5L45 40H55L60 30Z',
      'storm': 'M85 35C76.15 35 69 42.15 69 51C68.45 51 67.9 51 67.35 51.05C65.35 43.8 58.85 38.5 51 38.5C41.05 38.5 33 46.55 33 56.5C33 57.05 33 57.6 33.05 58.15C28.9 59.7 26 63.65 26 68.5C26 74.3 30.7 79 36.5 79H80.5C89.05 79 96 72.05 96 63.5C96 55.4 89.6 48.8 81.7 48.05C83.55 44.75 85 40.55 85 35ZM55 50L45 70H55L50 85L60 65H50L55 50Z',
      // New icons
      'graduation-cap': 'M60 20L15 40L60 60L95 45V70H105V40L60 20ZM30 48.5V68.5C30 75 42.5 85 60 85C77.5 85 90 75 90 68.5V48.5L60 62L30 48.5Z',
      ...
      'heart': 'M34 20C22.95 20 14 28.95 14 40C14 51.05 22.95 60 34 60H42L60 78L78 60H86C97.05 60 106 51.05 106 40C106 28.95 97.05 20 86 20C79.75 20 74.35 23.25 71.5 28.25C68.65 23.25 63.25 20 57 20C50.75 20 45.35 23.25 42.5 28.25C39.65 23.25 34.25 20 28 20Z',
      'government': 'M60 15L20 35V40H100V35L60 15ZM30 45V85H35V50H45V85H50V50H60V85H65V50H75V85H80V50H90V85H95V45H30Z',
      'flask': 'M42.5 20C40.15 20 38.25 21.9 38.25 24.25V30H45V24.25C45 21.9 43.1 20 40.75 20H42.5ZM52.5 20V30H59.25V24.25C59.25 21.9 57.35 20 55 20H52.5ZM67.5 20C65.15 20 63.25 21.9 63.25 24.25V30H70V24.25C70 21.9 68.1 20 65.75 20H67.5ZM77.5 20V30H84.25V24.25C84.25 21.9 82.35 20 80 20H77.5ZM25 35V40H95V35H25ZM30 45V85H90V45H30ZM40 55H80V75H40V55Z',
      ...
      'computer': 'M20 25C17.25 25 15 27.25 15 30V75C15 77.75 17.25 80 20 80H50V85H40C37.25 85 35 87.25 35 90C35 92.75 37.25 95 40 95H80C82.75 95 85 92.75 85 90C85 87.25 82.75 85 80 85H70V80H100C102.75 80 105 77.75 105 75V30C105 27.25 102.75 25 100 25H20ZM25 35H95V70H25V35Z',
      'newspaper-sharp': 'M20 20V100H100V30H90V90H30V20H20ZM40 30V40H80V30H40ZM40 50V55H80V50H40ZM40 60V65H80V60H40ZM40 70V75H65V70H40Z'
    };
    
    const path = iconPaths[iconName] || iconPaths['newspaper-sharp'];
    console.log('Icon path for', iconName, ':', path ? 'found' : 'using fallback');
    return path;
  };
    ...
    <div className="h-full rounded-lg overflow-hidden relative">
      ...
      >
        ...
        {locations.map((location) => {
          ...
          const categoryKey = primaryCategory ? getCategoryKeyByName(primaryCategory) : 'other_incidents';
          ...
          const icon = createCategoryIcon(primaryCategory || 'Diğer Olaylar', categoryColor);
          
          // Skip marker if icon creation failed (L not ready)
          if (!icon) {
            console.warn('Failed to create icon for location:', location.name, 'category:', primaryCategory);
            return null;
          }
